...
Matrix4 Matrix4::MakeFrustum(float l, float r, float b, float t,float n, float f)//Frustum  like glFrustum
{
    Matrix4 mat;
    mat[0]  = 2 * n / (r - l);
    mat[2]  = (r + l) / (r - l);
...
Matrix4 Matrix4::MakeFrustum(float fovY,float aspect, float n , float f)//like gluPerspective
{
    float tangent = tanf(fovY/2 * DEG2RAD); // fovY的半角
    float height = n * tangent;         // 近平面半高
    float width = height * aspect;          // 近平面半宽
    
    // 参数: left, right, bottom, top, near, far
    return MakeFrustum(-width, width, -height, height, n, f);
}

Matrix4 Matrix4::MakeOrtho(float l, float r, float b, float t, float n, float f)//like glOrtho
{
    Matrix4 mat;
    mat[0]  = 2 / (r - l);
    mat[3]  = -(r + l) / (r - l);
